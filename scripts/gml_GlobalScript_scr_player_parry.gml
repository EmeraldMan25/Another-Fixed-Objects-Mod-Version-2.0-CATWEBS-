function scr_player_parry() //gml_Script_scr_player_parry
{
    ...
    if ((parry_count > 0) && parry_lethal)
    {
        ...
        with (obj_baddie)
        {
            if (((other.parryID == id) || (distance_to_object(other) <= parry_threshold)) && (state != (137 << 0)) && (state != (4 << 0)) && (state != (138 << 0)) && parryable && (!(((state == (138 << 0)) && (thrown == true)))))
            {
                notification_push((4 << 0), [id, object_index, room])
